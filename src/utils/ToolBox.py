...
def impute_data(df, feature,ref1, ref2,  strategy = "mode", value = None):
    '''
    ...
    ref1 (str): nombre de la primera variable de referencia para la imputación.
    
    ref2 (str): nombre de la segunda variable de referencia para la imputación.
    ...
    for col in df.index:
        if df[feature][col]== None or pd.isna(df[feature][col]):
            df[feature][col] = df.loc[(df[ref1] == df[ref1][col]) & (df[ref2] == df[ref2][col]),feature].mode()[0] if strategy == "mode" else \
